 ...
 * <<#rel5.0:library_upgrades,Several libraries>> used by Gradle have been updated.
 ...
 * Please remove the `--add-modules java.xml.bind` option from `org.gradle.jvmargs`, if set. Gradle now bundles <<#rel5.0:jaxb_and_java9, JAXB>> for Java 9 and above.
 ...
 * The <<#rel5.0:configuration_avoidance, Configuration Avoidance API>> has been updated to prevent creating and configuring tasks that are never used.
 ...
 * Overriding default tasks <<#deprecations_4.8,deprecated in 4.8>> now produces an error.
+
...
+
See the https://github.com/gradle/kotlin-dsl/releases/tag/v1.0-RC13#breaking-changes[Gradle Kotlin DSL release notes] for more information and how to fix builds broken by the changes described above.
...
=== [[deprecations_4.8]] Deprecations
Prior to this release, builds were allowed to replace tasks that may be automatically created. This feature has been deprecated(https://docs.gradle.org/4.8/release-notes.html#overwriting-gradle's-built-in-tasks).
